    ...
    try {
      if (!req.body.mail) {
        ...
        User.findOne({ mail: req.body.mail }).then(async user => {
          if (user) {
            const token = jwt.sign({ data: user }, process.env.TOKENCODE, {
              expiresIn: '168h',
            });
            ...
            if (result) {
              res
                .status(200)
                .json({ message: 'We sent you password reset mail' });
            } else {
              res
                .status(400)
                .json({ message: 'faild to resetPassword try again later' });
            }
          } else {
      ...
      if (!req.body.password || !req.params.token) {
        ...
        await User.updateOne(
          ...
          .catch(error => {
            res
              .status(400)
              .json({ message: 'ERROR WHILE SETTING THE NEW PASSWORD', error });
          });
